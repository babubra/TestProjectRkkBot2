...
def copy_files_and_create_structure():
    ...
    app_bot_dir = os.path.join(project_root, "app_bot")
    dest_dir = os.path.join(script_dir, "export_files_app_bot")
    structure_file = "project_structure.txt"

    # Проверяем существование папки app_bot
    if not os.path.exists(app_bot_dir):
        print(f"Папка app_bot не найдена: {app_bot_dir}")
        return
    ...
    # Проход по всем файлам и папкам в app_bot
    for root, dirs, files in os.walk(app_bot_dir):
        ...
        if is_hidden_path(root, app_bot_dir):
            continue
        ...
        for file in files:
            ...
            relative_path = os.path.relpath(src_file, app_bot_dir)
            ...
            try:
                ...
                if final_filename != file:
                    structure.append(f"app_bot/{relative_path} -> {final_filename}")
                else:
                    structure.append(f"app_bot/{relative_path}")
    ...
    with open(structure_path, "w", encoding="utf-8") as f:
        f.write("СТРУКТУРА ПРОЕКТА APP_BOT\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"Исходная папка: {app_bot_dir}\n")
    ...
    print("Копирование app_bot завершено!")
    print(f"Исходная папка: {app_bot_dir}")
