...
def get_non_working_days_re(html_content: str) -> list[datetime.date]:
    """
...
def fetch_holidays_from_url(url: str) -> list[datetime.date]:
    """
    ...
    try:
        ...
        return get_non_working_days_re(html_content)
...
if __name__ == "__main__":
    # URL производственного календаря на 2026 год
    url_2026 = "https://www.consultant.ru/law/ref/calendar/proizvodstvennye/2025/"

    print(f"Загрузка данных с {url_2026}...")

    # Получаем список нерабочих дней
    holidays_2026 = fetch_holidays_from_url(url_2026)

    if holidays_2026:
        print(f"\nНайдено нерабочих дней для 2026 года: {len(holidays_2026)}")
    else:
        print("Не удалось получить данные о нерабочих днях.")
