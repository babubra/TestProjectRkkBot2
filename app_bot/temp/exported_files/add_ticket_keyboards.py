...
def get_add_ticket_time_kb(
    ...
    builder.button(
        ...
        callback_data=AddTicketTimeCallback(
            action="select_time",
            time_str="any-time",  # Специальное значение
        ).pack(),
    )
...
def get_add_ticket_cancel_kb() -> InlineKeyboardMarkup:
    """
    Создает инлайн-клавиатуру с единственной кнопкой "Отмена".
    Используется на шагах, где требуется текстовый ввод от пользователя.
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="❌ Отмена", callback_data="add_ticket_cancel")
    return builder.as_markup()
...
def get_add_ticket_files_kb() -> InlineKeyboardMarkup:
    """
    Создает инлайн-клавиатуру для шага прикрепления файлов при создании заявки.
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Завершить (файлы добавлены)", callback_data="add_ticket_files_done")
    builder.button(
        text="➡️ Пропустить (создать без файлов)", callback_data="add_ticket_skip_files"
    )
    builder.button(text="❌ Отмена", callback_data="add_ticket_cancel")
    builder.adjust(1)
    return builder.as_markup()
...
def get_add_ticket_confirmation_kb() -> InlineKeyboardMarkup:
    """
    Создает инлайн-клавиатуру для финального подтверждения создания заявки.
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Создать заявку", callback_data="add_ticket_confirm_create")
    builder.button(text="❌ Отмена", callback_data="add_ticket_cancel")
    builder.adjust(1)
    return builder.as_markup()
